...
echo "Installing AwesomeWM (one-dark-80s) on Gentoo"
...
if [ -f "$HOME/.config/awesome/themes/one-dark-80s/theme.lua" ]; then
    ln -sf "$HOME/.config/awesome/themes/one-dark-80s" "$HOME/.config/awesome/theme"
fi
...
echo "AwesomeWM one-dark-80s theme installed successfully!"
